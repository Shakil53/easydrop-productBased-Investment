...
const Sidebar = () => {
  ...
  const navItems = [
    { to: "/productBased-investment/home", icon: FaHome, label: "Home" },
    {
      to: "/productBased-investment/analytics",
      icon: IoMdAnalytics,
      label: "Analytics",
    },
    {
      to: "/productBased-investment/pricing",
      icon: HandCoins,
      label: "Pricing",
    },
    {
      to: "/productBased-investment/wallet",
      icon: DollarSign,
      label: "Wallet",
    },
  ];
  ...
  return (
    <aside className="bg-[#f8fafc] col-span-2 h-full flex flex-col sticky left-0 top-0 overflow-auto p-4 lg:p-5">
        ...
        <nav className="flex flex-col gap-4">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "md:p-3 bg-white rounded-sm border shadow-sm font-semibold text-black transition-all flex gap-2",
                  {
      ...
      <div className="hidden md:block bg-[#EBEEFF] max-w-lg p-4 shadow-md rounded-lg mt-12 relative">
        ...
        <h1 className="text-xs font-bold mb-2 mt-2 text-center">
          Need Help with Easydrop
        </h1>
        <div className="flex justify-center">
          <button className="rounded-sm bg-[#4338CA] text-white p-2 text-xs font-semibold">
            <Link to="/">Go to Help Center</Link>
          </button>
        </div>
      </div>
    </aside>
